    ...
    <AnimatePresence>
      {visible ? (
        ...
        >
          ...
          <div className="particle-field absolute inset-0 opacity-75" />
          <div className="relative flex h-full flex-col items-center justify-center">
            <motion.div
              initial={{ scale: 0.78, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              transition={{ duration: 0.7, ease: 'easeOut' }}
              className="relative h-28 w-28 overflow-hidden rounded-[2rem] border border-white/10 bg-[#f4ead1] shadow-[0_30px_80px_rgba(0,0,0,0.35)]"
            >
              <Image src="/legend-logo.png" alt="The Legend Detailers" fill sizes="112px" className="object-contain p-2" />
            </motion.div>
            <motion.div
              initial={{ scale: 0.85, opacity: 0.2 }}
              animate={{ scale: 1, opacity: 1 }}
              transition={{ duration: 0.8, ease: 'easeOut' }}
              className="mt-8 rounded-full border border-[#FFD700]/30 px-5 py-2 text-[10px] font-black uppercase tracking-[0.38em] text-[#FFD700]"
            >
              The Legend Detailers
            </motion.div>
            <motion.h1
